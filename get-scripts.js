...
function displayMine() {
    ...
    .then(function (response) { //chaining another. then onto the promise that accepts the response and console logging it
        console.log(response)

        let display = document.getElementById('journals') //set up a variable that refers back to the div with the id of "journals"
        for (i = 0; i = display.childNodes.length; i++) { //creating a for loop that checks if there are any elements in that div and removes them before displaying more
            display.removeChild(display.firstChild)
        }

        if(response.length === 0) { //checking to see if there are no posts and displaying something to let the user know

            let display = document.getElementById('journals') //referring to the journals div in the html
            let header = document.createElement('h5') 

            display.appendChild(header) //create the header, give it some text, and a class
